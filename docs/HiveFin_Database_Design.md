...
- **reconciliation_accounts** — `entity_id`, `ledger_account_id(uuid, references an existing asset-type LedgerAccount, never a second accounting account)`, `currency` (immutable once created), `display_name`, `masked_bank_identifier`, `reconciliation_enabled(bool, default true)`, `version`. Unique(`entity_id`, `ledger_account_id`). `ledger_account_id`/`entity_id`/`currency` never change after creation (a change is a new row).
- **bank_reconciliations** — `entity_id`, `reconciliation_account_id(fk)`, `period_ref`, `opening_balance`, `closing_balance`, `state` (`Draft`/`InProgress`/`PendingCompletionApproval`/`Completed`/`Reopened`), `source_data_watermark(timestamptz, nullable — set once at Completed and never altered thereafter, mirrors `report_runs.source_data_watermark`)`, `content_hash(nullable, set once at Completed)`, `completed_by`/`completed_at`(nullable), `reopened_by`/`reopened_at`(nullable), `version`, audit columns.
- **reconciliation_import_batches** — `reconciliation_id(fk)`, `reconciliation_account_id(fk)`, `file_hash`, `column_mapping(json)`, `imported_by`, `imported_at`, `line_count`. Append-only. Unique(`reconciliation_account_id`, `file_hash`) — the whole-file duplicate-import guard (API Contracts §14.5).
- **reconciliation_statement_lines** — `reconciliation_id(fk)`, `import_batch_id(fk)`, `source_line_identity`, `transaction_date`, `narration`, `normalized_narration` (generated: case-folded, whitespace-collapsed), `amount` (signed), `currency`, `external_bank_reference(nullable)`, `status` (`Unreconciled`/`Suggested`/`Matched`/`Reconciled`/`Unexplained`), `matched_allocation_ids(json, nullable)`, `resolved_by_journal_entry_id(uuid, nullable)`, `version`. Duplicate-identity unique index on (`reconciliation_account_id`, `transaction_date`, `amount`, `currency`, `normalized_narration`, `COALESCE(external_bank_reference, '')`) — null-safe by construction, scoped to the account across all its reconciliations, not just the current batch (API Contracts §14.5).
- **reconciliation_match_suggestions** — `statement_line_id(fk)`, `rank`, `total_amount`, `currency`, `reference_match(bool)`, `superseded(bool, default false — set true when a later GenerateMatchSuggestions run replaces it)`. **reconciliation_match_suggestion_allocations** — `suggestion_id(fk)`, `allocation_id(uuid)`. One-to-one/one-to-many/many-to-one only (API Contracts §14.6) — never many-to-many.
- Index: `reconciliation_statement_lines(reconciliation_id, status)`; the duplicate-detection unique index above; `bank_reconciliations(reconciliation_account_id, period_ref)`; `reconciliation_match_suggestions(statement_line_id, superseded)`.
- No cross-context foreign key exists from any `reconciliation_*` table to `settlement_allocations`/`journal_entries`/`journal_lines` rows (AP-001 — UUID references only, e.g. `matched_allocation_ids`, `resolved_by_journal_entry_id`).
- PostgreSQL immutability: `bank_reconciliations` rows in `Completed` state are protected from `opening_balance`/`closing_balance`/`source_data_watermark`/`content_hash` mutation and deletion (trigger, mirrors `report_runs`' `protect_report_run_fact` pattern) — only `state`/`version`/`reopened_by`/`reopened_at` may change thereafter, on `ReopenReconciliation`. `reconciliation_statement_lines` rows in `Reconciled` state are protected from `amount`/`currency`/`transaction_date`/`matched_allocation_ids`/`resolved_by_journal_entry_id` mutation and deletion; only `status`/`version` may change (never back to `Reconciled` once left, except via a fresh `ConfirmMatch`/bank-only-entry cycle after `ReopenReconciliation` re-admits the batch). `reconciliation_import_batches` is append-only (no UPDATE/DELETE grant).
