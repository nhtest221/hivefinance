...
3. **Value objects:** `DocumentId`, `DocumentNumber`, `Money`, `TaxSnapshot`(per line), `ExchangeRateReference`(invoice-date), `CustomerRef`(UUID), `DueDate`, `PaymentInstructionsRef`, `Disposition`(CreditNote: Applied/Held/Refunded), `ReasonCode`(CreditNote), `openBalance`(Money), `AuditStamp`.
4. **Boundary:** document header + lines + its `openBalance`. Excludes customer master, allocations, ledger.
5. **Invariants:** VAT defaults from customer type, line-overridable; recognition posts at **issue date**; **issued ⇒ immutable**; **void only if all 4 safe-window conditions** (unpaid, current open period, not in filed VAT return, no downstream allocations); `openBalance` = issued − Σ settled, ≥ 0; **CreditNote VAT adjustment in the note's period**; CreditNote reason code mandatory.
6. **Lifecycle (Invoice):** Draft → Sent(=recognised) → Partially Paid → Paid; branch → Void; auto Overdue. **CreditNote:** Draft → Posted → (Applied/Held/Refunded).
7. **Commands:** CreateInvoiceDraft, IssueInvoice, VoidInvoice, ApplySettlement(amount, expectedVersion) [§0.3], IssueCreditNote, SetCreditNoteDisposition.
8. **Events:** InvoiceIssued, InvoiceVoided, InvoiceStatusChanged, CreditNoteIssued, CreditNoteDispositionSet.
9. **Repository:** InvoiceRepository, CreditNoteRepository (by DocumentId; query GetOpenReceivable(byDocumentId)).
...
2. **Entities:** BillLine / (DebitNote lines) / Expense (single-bodied).
...
5. **Invariants:** **SBU weights Σ = 1.0000**; recognition at **approval**; input VAT recoverable→asset / non-recoverable→cost; AIT/VDS captured; **approved ⇒ immutable**; void via 4-condition test else Debit Note; `openBalance` = approved − Σ settled, ≥ 0; Expense cash-settled posts to bank, accrued posts to AP.
6. **Lifecycle (Bill):** Draft → Awaiting Payment(=approved) → Partially Paid → Paid; → Void; auto Overdue. Expense: Draft → Recorded (cash or accrued). DebitNote: Draft → Posted → applied.
7. **Commands:** CreateBillDraft, ApproveBill, VoidBill, ApplySettlement(amount, expectedVersion), IssueDebitNote, RecordExpense.
8. **Events:** BillApproved, BillVoided, BillStatusChanged, DebitNoteIssued, ExpenseRecorded.
9. **Repository:** BillRepository, DebitNoteRepository, ExpenseRepository (GetOpenPayable(byDocumentId)).
...
### AccountingPeriod
1. **Root:** AccountingPeriod (entity × period). 2. **Entities:** PeriodTransition (log child). 3. **VOs:** PeriodRef, `State`(Open/SoftClose/HardClose/Reopened), VATLockStatus, ReasonCode, AuditStamp. 4. **Boundary:** period state + transition log. 5. **Invariants:** exactly one state; Soft Close **repeatable**; Hard Close **irreversible via normal ops**; **reopen requires role+reason+management-approval+user-notification+re-close**; corrections to closed periods route to the current open period. 6. **Lifecycle:** Open → SoftClose(↺) → HardClose → Reopened → (re-close). 7. **Commands:** OpenPeriod, SoftClose, HardClose, Reopen, LockVAT, RollYearEnd. 8. **Events:** PeriodSoftClosed, PeriodHardClosed, PeriodReopened, VATPeriodLocked, YearEndRolled. 9. **Repo:** AccountingPeriodRepository; **Query:** IsDatePostable (OHS). 12. **Concurrency:** low; transitions require four-eyes (Identity). 13. **Refs (UUID):** EntityRef, FiscalCalendar. 14. **Size:** right.
### FiscalCalendar
