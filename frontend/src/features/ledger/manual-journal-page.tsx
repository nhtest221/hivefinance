...
import { Alert, Button, Card, CardContent, CardHeader, Input, PageHeader } from '@/design-system'
...
export function ManualJournalPage() {
  ...
  return (
    <AppLayout>
      <PageHeader title="Manual Journal" description="Draft, post, and inspect the General Ledger for a balanced manual entry." />
      <div className="space-y-4 p-4 lg:p-6">
        <Card><CardHeader><h2 className="font-semibold">Balanced entry</h2></CardHeader><CardContent><form className="grid gap-4 md:grid-cols-2" onSubmit={submit}>
          ...
          <label className="text-sm">Debit account<select className="h-9 w-full rounded-md border border-[var(--color-border)] bg-[var(--color-surface)] px-3 text-sm" value={debitAccount} onChange={(e) => setDebitAccount(e.target.value)} required><option value="">Select</option>{accounts.map((a) => <option key={a.id} value={a.id}>{a.code} — {a.name}</option>)}</select></label>
          <label className="text-sm">Credit account<select className="h-9 w-full rounded-md border border-[var(--color-border)] bg-[var(--color-surface)] px-3 text-sm" value={creditAccount} onChange={(e) => setCreditAccount(e.target.value)} required><option value="">Select</option>{accounts.map((a) => <option key={a.id} value={a.id}>{a.code} — {a.name}</option>)}</select></label>
        ...
        {ledger ? <Card><CardHeader><h2 className="font-semibold">General Ledger — {ledger.account.code} {ledger.account.name}</h2></CardHeader><CardContent><div className="space-y-2 text-sm">{ledger.entries.map((entry) => <div className="grid grid-cols-4 gap-2 border-b border-[var(--color-border)] py-2" key={entry.journal_entry_id}><span>{entry.entry_date}</span><span>{entry.description}</span><span className="tabular-nums">{entry.debit?.amount ?? entry.credit?.amount}</span><span className="tabular-nums">{entry.running_balance.amount}</span></div>)}<p className="font-medium">Closing: {ledger.closing_balance.amount} {ledger.closing_balance.currency}</p></div></CardContent></Card> : null}
      </div>
    </AppLayout>
  )
}
