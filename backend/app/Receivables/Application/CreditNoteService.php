...
use App\Models\Receivables\CreditNoteLine;
...
/**
 * M4A — Credit Note create/edit/detail/list/post (API Contracts §12.3.1-12.3.5;
 * Aggregate Design "ReceivableDocument"; Repository Contracts CreditNoteRepository).
 * Disposition (apply/hold/refund/reverse) lives in CreditNoteDispositionService.
 */
final readonly class CreditNoteService
{
    ...
    {
        if ($denied = $this->commands->authorize($actor, $entityId, 'receivables.credit_notes.create')) {
            return $denied;
        }
        if ($error = $this->commands->requireIdempotency($key)) {
            return $error;
        }
        $op = 'POST /v1/credit-notes';
        ...
        return DB::transaction(function () use ($actor, $entityId, $data, $prepared, $key, $op, $hash): DocumentActionResult {
            $note = $this->notes->addDraft([
                'entity_id' => $entityId, 'provisional_token' => (string) Str::uuid(), 'customer_id' => $prepared['invoice']->customer_id,
                'source_invoice_id' => $data['source_document_id'], 'source_document_expected_version' => $data['source_document_expected_version'],
                'note_date' => $data['note_date'], 'currency' => $prepared['invoice']->currency, 'reason_code' => $data['reason_code'],
                'narrative' => $data['narrative'] ?? null, 'source_rate_record_id' => $prepared['invoice']->rate_record_id,
                'source_exchange_rate_reference' => $prepared['invoice']->exchange_rate_reference, 'proposed_total' => $prepared['proposed_total'],
            ...
            $body = ['credit_note' => $this->present($note)];
            $this->audit->record('receivables', 'credit_note_draft_created', 'credit_note', $note->id, $actor->id, $entityId, after: $this->safe($note), correlationId: $this->correlation());
            $this->outbox->record('CreditNoteCreated', 'CreditNote', $note->id, ['creditNoteId' => $note->id, 'partyId' => $note->customer_id, 'sourceDocumentId' => $note->source_invoice_id], $entityId);
    ...
    public function update(User $actor, string $entityId, string $id, array $data, ?string $key, ?string $ifMatch): DocumentActionResult
    {
        ...
        if ($note->state !== 'draft') {
            return $this->commands->error('invariant_violation', 'Only draft credit notes may be updated.', 422, ['rule' => 'note_not_draft']);
        }
        ...
        return DB::transaction(function () use ($actor, $entityId, $note, $merged, $prepared, $expected, $key, $op, $hash): DocumentActionResult {
            ...
            $saved = $this->notes->saveDraft($entityId, $note->id, [
                'customer_id' => $prepared['invoice']->customer_id, 'source_invoice_id' => $merged['source_document_id'],
                'source_document_expected_version' => $merged['source_document_expected_version'], 'note_date' => $merged['note_date'],
                'currency' => $prepared['invoice']->currency, 'reason_code' => $merged['reason_code'], 'narrative' => $merged['narrative'] ?? null,
                'source_rate_record_id' => $prepared['invoice']->rate_record_id, 'source_exchange_rate_reference' => $prepared['invoice']->exchange_rate_reference,
            ...
            $body = ['credit_note' => $this->present($saved)];
            $this->audit->record('receivables', 'credit_note_draft_updated', 'credit_note', $saved->id, $actor->id, $entityId, $before, $this->safe($saved), correlationId: $this->correlation());
    ...
    {
        if ($denied = $this->commands->authorize($actor, $entityId, 'receivables.credit_notes.post')) {
            return $denied;
        }
        if ($error = $this->commands->requireIdempotency($key)) {
            return $error;
        ...
        if ($this->approvalPolicy->isConfigured($entityId)) {
            ...
            $result = $this->approvals->requestApproval($actor, $entityId, new OriginatingCommand('credit_note_post', 1, $payload, $id, 'receivables.credit_notes.post', $expected), 'credit_note_post:'.$id, (string) $key, $this->correlation() ?? (string) Str::uuid());

            return new DocumentActionResult($result->payload, $result->status, $result->headers);
        }
        $note = $this->notes->getById($entityId, $id);
        if ($note !== null && $note->created_by === $actor->id) {
            return $this->commands->error('sod_exception_required', 'The credit note maker cannot directly post the credit note.', 403);
        }
    ...
    public function executeApproved(string $type, array $payload, ApprovalExecutionContext $context): DocumentActionResult
    {
        ...
        if (! is_string($id) || ! is_int($expected) || ! is_string($key)) {
            return $this->commands->error('validation', 'The approved credit note payload is invalid.', 400);
        }
        ...
        return match ($type) {
            ...
            default => $this->commands->error('validation', 'Unsupported CreditNote command.', 400),
        };
    }

    public function show(User $actor, string $entityId, string $id): DocumentActionResult
    {
        if ($denied = $this->commands->authorize($actor, $entityId, 'receivables.credit_notes.read')) {
            return $denied;
        }
        $note = $this->query->getDetail($entityId, $id);

        return $note ? new DocumentActionResult(['credit_note' => $this->presentDetail($note)]) : $this->notFound();
    }
    ...
    public function list(User $actor, string $entityId, array $filters): DocumentActionResult
    {
        ...
        return new DocumentActionResult(['credit_notes' => $page->getCollection()->map(fn (CreditNote $n): array => $this->summary($n))->all(), 'page' => ['limit' => $limit, 'next_cursor' => StableCursor::encode($page->nextCursor(), $boundary, $binding)]]);
    }
    ...
    private function preflightPost(string $entityId, string $id, int $expected): ?DocumentActionResult
    {
        ...
        if ($note->state !== 'draft') {
            return $this->commands->error('invariant_violation', 'Only draft credit notes may be posted.', 422, ['rule' => 'note_not_draft']);
        }
        if ($note->version !== $expected) {
            return $this->conflict($note->version);
        }
    ...
    {
        if ($replay = $this->commands->replay($makerId, $entityId, 'credit_note_post:'.$id, $key, $hash)) {
            return $replay;
        }
        $note = $this->notes->getById($entityId, $id);
        if ($note === null) {
        ...
        if ($note->state !== 'draft' || $note->version !== $expected) {
            return $note->state !== 'draft' ? $this->commands->error('invariant_violation', 'Only draft credit notes may be posted.', 422, ['rule' => 'note_not_draft']) : $this->conflict($note->version);
        }
        $invoice = Invoice::query()->where('entity_id', $entityId)->find($note->source_invoice_id);
        if ($invoice === null || $invoice->version !== $note->source_document_expected_version) {
            return $this->commands->error('invariant_violation', 'The source invoice has changed since the draft was created.', 422, ['rule' => 'source_document_version_conflict']);
        }
        $revenueAccount = config('documents.invoice.revenue_account_id');
        $creditAccount = config('settlement.accounts.customer_credit');
        if (! is_string($revenueAccount) || ! Str::isUuid($revenueAccount) || ! is_string($creditAccount) || ! Str::isUuid($creditAccount)) {
            return $this->commands->error('missing_posting_configuration', 'Credit note account mapping is unavailable.', 422);
        }
        $draw = $this->numbers->draw('credit_note', $entityId, (string) $note->note_date->toDateString());
        if ($draw === null) {
            return $this->commands->error('missing_numbering_configuration', 'Credit note numbering configuration is unavailable.', 422);
        }
        try {
            return DB::transaction(function () use ($note, $entityId, $revenueAccount, $creditAccount, $draw, $expected, $key, $hash, $makerId, $executorId, $causationId): DocumentActionResult {
                $reference = $note->source_exchange_rate_reference;
                $postingLines = $this->postingLines($note, $revenueAccount, $creditAccount, $reference);
                if ($postingLines === null) {
                    return $this->commands->error('missing_posting_configuration', 'Tax account mapping is unavailable.', 422);
                }
                $posted = $this->posting->post($entityId, $note->id, $note->note_date->toDateString(), 'credit_note', $executorId, $postingLines);
                if ($posted->errorCode !== null || $posted->journalId === null) {
                    return $this->postingError($posted->errorCode);
                }
                $total = $note->lines->sum('total_amount');
                $totalStr = ExactDecimal::normalize((string) $total);
                ...
                $body = ['credit_note' => $this->present($updated)];
                $this->audit->record('receivables', 'credit_note_posted', 'credit_note', $updated->id, $executorId, $entityId, before: ['state' => 'draft', 'version' => $expected], after: $this->safe($updated), metadata: ['maker_id' => $makerId], correlationId: $this->correlation());
                $this->outbox->record('CreditNoteIssued', 'CreditNote', $updated->id, ['partyType' => 'customer', 'documentType' => 'invoice', 'partyId' => $updated->customer_id, 'sourceDocumentId' => $updated->source_invoice_id, 'documentNumber' => $updated->document_number, 'postedAmount' => $this->money($updated->posted_amount, $updated->currency), 'appliedAmount' => $this->money('0.0000', $updated->currency), 'refundedAmount' => $this->money('0.0000', $updated->currency), 'heldRemainingAmount' => $this->money('0.0000', $updated->currency), 'undisposedAmount' => $this->money($updated->undisposed_amount, $updated->currency), 'periodRef' => $updated->period_ref, 'sourceRateRecordId' => $updated->source_rate_record_id, 'journalEntryIds' => $updated->journal_entry_ids], $entityId, 2, ['causation_id' => $causationId]);
                $this->commands->store($makerId, $entityId, 'credit_note_post:'.$updated->id, $key, $hash, 201, $body);
     ...
     */
    private function postingLines(CreditNote $note, string $revenueAccount, string $creditAccount, ?array $reference): ?array
    {
        $taxGroups = [];
        foreach ($note->lines as $line) {
        ...
        $net = ExactDecimal::normalize((string) $note->lines->sum('net_amount'));
        $total = ExactDecimal::normalize((string) $note->lines->sum('total_amount'));
        $functionalNet = $this->functional($net, $reference);
        $functionalTotal = $this->functional($total, $reference);
        if ($functionalNet === null || $functionalTotal === null) {
            return null;
        }
        $lines = [];
        $functionalTaxSum = '0.0000';
        foreach ($taxGroups as $account => $amount) {
            $converted = $this->functional($amount, $reference);
            if ($converted === null) {
                return null;
            }
            $functionalTaxSum = ExactDecimal::add($functionalTaxSum, $converted);
            $lines[] = $this->postingLine($account, 'Credit note output tax reversal', $converted, '0.0000', $note->currency, $amount, $reference);
        }
        $lines[] = $this->postingLine($revenueAccount, 'Credit note revenue reversal', $functionalNet, '0.0000', $note->currency, $net, $reference);
        $lines[] = $this->postingLine($creditAccount, 'Customer credit issued', '0.0000', ExactDecimal::add($functionalNet, $functionalTaxSum), $note->currency, $total, $reference);
    ...
    /** @param array<string,mixed> $data
     * @return array{invoice:Invoice,lines:list<array<string,mixed>>,proposed_total:string,period_ref:?string}|DocumentActionResult
     */
    private function prepare(string $entityId, array $data): array|DocumentActionResult
    {
        ...
        if ($partyType !== 'customer' || $documentType !== 'invoice') {
            return $this->commands->error('invariant_violation', 'Credit Note direction is fixed to customer/invoice.', 422, ['rule' => 'note_direction_mismatch']);
        }
        $invoice = Invoice::query()->with('lines')->where('entity_id', $entityId)->find($data['source_document_id']);
        if (! $invoice instanceof Invoice || $invoice->status !== 'sent') {
            return $this->commands->error('invariant_violation', 'The source invoice must be an entity-owned, issued, non-void invoice.', 422, ['rule' => 'invalid_source_document']);
        }
        if ((string) $invoice->customer_id !== (string) $data['party_id']) {
            return $this->commands->error('invariant_violation', 'The credit note party must match the source invoice customer.', 422, ['rule' => 'invalid_source_document']);
        }
        if ($invoice->version !== (int) $data['source_document_expected_version']) {
            return $this->commands->error('invariant_violation', 'The source invoice version has changed.', 422, ['rule' => 'source_document_version_conflict']);
        }
        ...
        foreach ($data['lines'] as $index => $line) {
            ...
            if ($sourceLine === null || ! ExactDecimal::positive((string) $line['net_amount']['amount'])) {
                return $this->commands->error('invariant_violation', 'Every line must reference a source invoice line with a positive net_amount.', 422, ['rule' => 'invalid_source_document']);
            }
            ...
            $lines[] = ['source_line_id' => $sourceLineId, 'line_no' => $index + 1, 'description' => $line['description'] ?? null, 'net_amount' => $netAmount, 'tax_snapshot' => $sourceLine->tax_snapshot, 'tax_amount' => $tax, 'total_amount' => $total];
        }
        $period = $this->periods->findForDate($entityId, (string) $data['note_date']);

        return ['invoice' => $invoice, 'lines' => $lines, 'proposed_total' => $proposedTotal, 'period_ref' => $period?->period_ref];
    }

    private function priorPostedTotal(string $entityId, string $sourceLineId): string
    {
        $sum = CreditNoteLine::query()->where('source_line_id', $sourceLineId)
            ->whereHas('creditNote', fn ($q) => $q->where('entity_id', $entityId)->where('state', 'posted'))
            ->sum('net_amount');
    ...
    private function applySourceSnapshot(?array $snapshot, string $netAmount, string $sourceNet, string $sourceTax): string
    {
        ...
        // Applies the exact copied historical rate/treatment to the (possibly partial)
        // corrected net_amount — never re-determines against the current Tax Pack
        // (API Contracts §12.2: "copies the source line TaxSnapshot").
    ...
    /** @return array<string,mixed> */
    private function requestData(CreditNote $note): array
    {
        return ['party_type' => 'customer', 'document_type' => 'invoice', 'party_id' => $note->customer_id, 'source_document_id' => $note->source_invoice_id, 'source_document_expected_version' => $note->source_document_expected_version, 'note_date' => $note->note_date->toDateString(), 'reason_code' => $note->reason_code, 'narrative' => $note->narrative, 'lines' => $note->lines->map(fn (CreditNoteLine $l): array => ['source_line_id' => $l->source_line_id, 'description' => $l->description, 'net_amount' => ['amount' => $l->net_amount, 'currency' => $note->currency]])->all()];
    }
    ...
    {
        return ['id' => $note->id, 'party_type' => 'customer', 'document_type' => 'invoice', 'party_id' => $note->customer_id, 'source_document_id' => $note->source_invoice_id, 'document_number' => $note->document_number, 'note_date' => $note->note_date->toDateString(), 'currency' => $note->currency, 'reason_code' => $note->reason_code, 'posted_amount' => $this->money($note->posted_amount, $note->currency), 'applied_amount' => $this->money($note->applied_amount, $note->currency), 'refunded_amount' => $this->money($note->refunded_amount, $note->currency), 'held_remaining_amount' => $this->money($note->held_remaining_amount, $note->currency), 'undisposed_amount' => $this->money($note->undisposed_amount, $note->currency), 'state' => $note->state, 'version' => $note->version];
    }

    /** @return array<string,mixed> */
    private function safe(CreditNote $note): array
    {
