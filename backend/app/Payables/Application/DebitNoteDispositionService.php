...
/**
 * M4A — Debit Note apply/hold/refund/reverse (API Contracts §§12.4.6-12.4.9). Mirror of
 * CreditNoteDispositionService with vendor/bill direction: Apply debits AP and credits
 * Vendor Credits control (the opposite sides of Credit Note's Customer Credits control
 * entry, since a debit note reduces what the entity owes the vendor rather than what a
 * customer owes the entity).
 */
final readonly class DebitNoteDispositionService
{
    ...
    public function executeApproved(string $type, array $payload, ApprovalExecutionContext $context): DocumentActionResult
    {
        ...
        if (! is_string($id) || ! is_int($expected) || ! is_string($key) || ! is_array($data)) {
            return $this->commands->error('validation', 'The approved debit note disposition payload is invalid.', 400);
        }
    ...
    private function submit(User $actor, string $entityId, string $type, string $id, array $data, ?string $key, ?string $ifMatch, string $capability): DocumentActionResult
    {
        ...
        if ($note->state !== 'posted') {
            return $this->commands->error('invariant_violation', $note->state === 'reversed' ? 'The debit note has been reversed.' : 'The debit note is not posted.', 422, ['rule' => $note->state === 'reversed' ? 'note_reversed' : 'note_not_posted']);
        }
        ...
        if ($this->approvalPolicy->isConfigured($entityId)) {
            ...
            $result = $this->approvals->requestApproval($actor, $entityId, new OriginatingCommand('debit_note_'.$type, 1, $payload, $id, $capability, $expected), 'debit_note_'.$type.':'.$id, (string) $key, $this->correlation() ?? (string) Str::uuid());
    ...
    private function execute(string $type, string $entityId, string $id, array $data, int $expected, string $key, string $makerId, string $executorId, string $causationId): DocumentActionResult
    {
        ...
        return match ($type) {
            ...
            default => $this->commands->error('validation', 'Unsupported DebitNote disposition command.', 400),
        };
    }
    ...
    private function executeHold(string $entityId, string $id, array $data, int $expected, string $key, string $hash, string $makerId, string $executorId, string $causationId): DocumentActionResult
    {
        return DB::transaction(function () use ($entityId, $id, $data, $expected, $key, $hash, $makerId, $executorId, $causationId): DocumentActionResult {
            $note = DebitNote::query()->where('entity_id', $entityId)->whereKey($id)->lockForUpdate()->first();
            ...
            $held = $this->tranches->holdFromNote($entityId, 'vendor', $note->vendor_id, $note->currency, $amount, $note->source_exchange_rate_reference, $note->id, $note->document_number);
            if (isset($held['error'])) {
            ...
            $body = ['debit_note' => $this->summary($updated), 'credit_sources' => [$this->presentTranche($held['tranche'])]];
            $this->audit->record('payables', 'debit_note_held', 'debit_note', $id, $executorId, $entityId, after: $body['debit_note'], metadata: ['maker_id' => $makerId], correlationId: $this->correlation());
            $this->outbox->record('DebitNoteHeld', 'DebitNote', $id, ['debitNoteId' => $id, 'transferredAmount' => $this->money($amount, $note->currency), 'resultingState' => $body['debit_note'], 'creditTrancheId' => $held['tranche']->id], $entityId, metadata: ['causation_id' => $causationId]);
            $this->outbox->record('CreditHeld', 'CreditTranche', $held['tranche']->id, ['creditTrancheId' => $held['tranche']->id, 'partyType' => 'vendor', 'partyId' => $note->vendor_id, 'money' => $this->money($amount, $note->currency), 'sourceNoteId' => $id], $entityId, 2, ['causation_id' => $causationId]);
            $this->commands->store($makerId, $entityId, 'debit_note_hold:'.$id, $key, $hash, 201, $body);
    ...
    private function executeApply(string $entityId, string $id, array $data, int $expected, string $key, string $hash, string $makerId, string $executorId, string $causationId): DocumentActionResult
    {
        return DB::transaction(function () use ($entityId, $id, $data, $expected, $key, $hash, $makerId, $executorId, $causationId): DocumentActionResult {
            ...
            $payableAccount = config('documents.bill.payable_account_id');
            if (! is_string($creditAccount) || ! Str::isUuid($creditAccount) || ! is_string($payableAccount) || ! Str::isUuid($payableAccount)) {
            ...
            $allocation = Allocation::query()->create(['entity_id' => $entityId, 'allocation_number' => null, 'operation' => 'credit_application', 'party_type' => 'vendor', 'party_id' => $note->vendor_id, 'settlement_date' => $data['application_date'], 'bank_account_id' => null, 'currency' => $note->currency, 'gross_amount' => $allocationTotal, 'bank_amount' => '0.0000', 'withholding_amount' => '0.0000', 'allocated_amount' => $allocationTotal, 'unapplied_amount' => '0.0000', 'functional_gross_amount' => '0.0000', 'rate_record_id' => null, 'exchange_rate_reference' => null, 'journal_entry_ids' => [], 'state' => 'building', 'version' => 1, 'created_by' => $makerId, 'posted_at' => Carbon::now('UTC')]);
            $consumedSources = [];
            $fxResults = [];
            $trancheIds = [];
            if ($source === 'held') {
                $result = $this->tranches->consume($entityId, 'vendor', $sources, null, $allocation->id, 'application');
            ...
            foreach ($data['allocations'] as $line) {
                ...
                $before = $this->payables->getOpenPayable($entityId, $documentId);
                if ($before === null || $before['party_id'] !== $note->vendor_id || $before['currency'] !== $note->currency) {
                    return $this->commands->error('document_party_mismatch', 'The target document is not eligible for this debit note.', 422);
                }
                $amount = ExactDecimal::normalize((string) $line['amount']['amount']);
                $applied = $this->payables->applySettlement($entityId, $documentId, $amount, (int) $line['expected_version']);
                ...
                $postingLines[] = $this->postingLine($payableAccount, 'Debit note applied to document', $functional, '0.0000', $note->currency, $amount, null);
                $allocation->links()->create(['entity_id' => $entityId, 'document_type' => 'bill', 'document_id' => $documentId, 'document_number' => $applied['before']['document_number'], 'document_party_id' => $applied['before']['party_id'], 'credit_tranche_id' => $trancheIds[0] ?? null, 'applied_amount' => $amount, 'expected_version' => (int) $line['expected_version'], 'open_balance_before' => $applied['before']['open_balance'], 'open_balance_after' => $applied['after']['open_balance'], 'version_before' => $applied['before']['version'], 'version_after' => $applied['after']['version'], 'status_before' => $applied['before']['status'], 'status_after' => $applied['after']['status'], 'document_rate_record_id' => $applied['before']['rate_record_id'] ?? null]);
                $applications[] = ['document_id' => $documentId, 'amount' => $this->money($amount, $note->currency), 'version_after' => $applied['after']['version']];
                $this->outbox->record('BillStatusChanged', 'Bill', $documentId, ['documentId' => $documentId, 'status' => $applied['after']['status'], 'openBalance' => $this->money((string) $applied['after']['open_balance'], $note->currency)], $entityId, metadata: ['causation_id' => $causationId]);
            }
            $postingLines[] = $this->postingLine($creditAccount, 'Vendor credit consumed', '0.0000', $functionalTotal, $note->currency, $allocationTotal, null);
            ...
            $updated = $this->notes->getById($entityId, $id);
            $body = ['debit_note' => $this->summary($updated), 'applications' => $applications, 'consumed_credit_sources' => $consumedSources, 'realised_fx_results' => $fxResults];
            $this->audit->record('payables', 'debit_note_applied', 'debit_note', $id, $executorId, $entityId, after: $body['debit_note'], metadata: ['maker_id' => $makerId], correlationId: $this->correlation());
            $this->outbox->record('DebitNoteApplied', 'DebitNote', $id, ['debitNoteId' => $id, 'source' => $source, 'money' => $this->money($allocationTotal, $note->currency), 'resultingState' => $body['debit_note']], $entityId, metadata: ['causation_id' => $causationId]);
            if ($source === 'held') {
                $this->outbox->record('CreditApplied', 'Allocation', $allocation->id, ['allocationId' => $allocation->id, 'partyType' => 'vendor', 'partyId' => $note->vendor_id, 'money' => $this->money($allocationTotal, $note->currency)], $entityId, 2, ['causation_id' => $causationId]);
            }
            $this->commands->store($makerId, $entityId, 'debit_note_apply:'.$id, $key, $hash, 201, $body);
    ...
    private function executeRefund(string $entityId, string $id, array $data, int $expected, string $key, string $hash, string $makerId, string $executorId, string $causationId): DocumentActionResult
    {
        ...
        try {
            return DB::transaction(function () use ($entityId, $id, $data, $expected, $key, $hash, $makerId, $executorId, $causationId, $draw): DocumentActionResult {
                ...
                $allocation = Allocation::query()->create(['entity_id' => $entityId, 'allocation_number' => $draw['number'], 'operation' => 'credit_refund', 'party_type' => 'vendor', 'party_id' => $note->vendor_id, 'settlement_date' => $data['refund_date'], 'bank_account_id' => $data['bank_account_id'], 'currency' => $note->currency, 'gross_amount' => $refundAmount, 'bank_amount' => $refundAmount, 'withholding_amount' => '0.0000', 'allocated_amount' => $refundAmount, 'unapplied_amount' => '0.0000', 'functional_gross_amount' => '0.0000', 'rate_record_id' => null, 'exchange_rate_reference' => null, 'journal_entry_ids' => [], 'state' => 'building', 'version' => 1, 'created_by' => $makerId, 'posted_at' => Carbon::now('UTC')]);
                $result = $this->tranches->consume($entityId, 'vendor', $sources, null, $allocation->id, 'refund');
                if (isset($result['error'])) {
                    return $this->commands->error($result['error']['code'], $result['error']['message'], $result['error']['status']);
                }
                $allocation->functional_gross_amount = $result['comparison_total'];
                $postingLines = [
                    $this->postingLine((string) $data['bank_account_id'], 'Vendor credit refund received', $result['comparison_total'], '0.0000', $note->currency, $refundAmount, null),
                    $this->postingLine($creditAccount, 'Vendor credit refunded', '0.0000', $result['functional_total'], $note->currency, $refundAmount, null),
                ];
                ...
                $updated = $this->notes->getById($entityId, $id);
                $body = ['debit_note' => $this->summary($updated), 'allocation' => ['id' => $allocation->id, 'operation' => 'credit_refund', 'state' => 'posted'], 'realised_fx_results' => $result['fx_results']];
                $this->audit->record('payables', 'debit_note_refunded', 'debit_note', $id, $executorId, $entityId, after: $body['debit_note'], metadata: ['maker_id' => $makerId], correlationId: $this->correlation());
                $this->outbox->record('DebitNoteRefunded', 'DebitNote', $id, ['debitNoteId' => $id, 'money' => $this->money($refundAmount, $note->currency), 'resultingState' => $body['debit_note'], 'settlementAllocationId' => $allocation->id], $entityId, metadata: ['causation_id' => $causationId]);
                $this->outbox->record('CreditRefunded', 'Allocation', $allocation->id, ['allocationId' => $allocation->id, 'partyType' => 'vendor', 'partyId' => $note->vendor_id, 'money' => $this->money($refundAmount, $note->currency)], $entityId, 2, ['causation_id' => $causationId]);
                $this->commands->store($makerId, $entityId, 'debit_note_refund:'.$id, $key, $hash, 201, $body);
    ...
    {
        return DB::transaction(function () use ($entityId, $id, $data, $expected, $key, $hash, $makerId, $executorId, $causationId): DocumentActionResult {
            $note = DebitNote::query()->with('dispositions')->where('entity_id', $entityId)->whereKey($id)->lockForUpdate()->first();
            if ($note === null || $note->state !== 'posted' || $note->version !== $expected) {
                return $this->reconflict($entityId, $id, $expected, $note);
            }
            ...
            foreach ($note->dispositions as $disposition) {
                if ($disposition->settlement_allocation_id !== null) {
                    ...
                    foreach ($allocation->links as $link) {
                        $current = $this->payables->getOpenPayable($entityId, $link->document_id);
                        if ($current === null) {
                        ...
                        $restored = $this->payables->reverseSettlement($entityId, $link->document_id, (string) $link->applied_amount, (int) $current['version']);
                        ...
                        $this->outbox->record('BillStatusChanged', 'Bill', $link->document_id, ['documentId' => $link->document_id, 'status' => $restored['after']['status'], 'openBalance' => $this->money((string) $restored['after']['open_balance'], $note->currency)], $entityId, metadata: ['causation_id' => $causationId]);
                    }
            ...
            $updated = $this->notes->getById($entityId, $id);
            $body = ['debit_note' => $this->summary($updated), 'reversal' => ['id' => $reversal->id, 'original_note_id' => $id, 'reversal_date' => $reversal->reversal_date->toDateString()], 'journal_entry_ids' => $allJournalIds];
            $this->audit->record('payables', 'debit_note_reversed', 'debit_note', $id, $executorId, $entityId, before: ['state' => 'posted', 'version' => $expected], after: $body['debit_note'], metadata: ['maker_id' => $makerId, 'reason_code' => $data['reason_code']], correlationId: $this->correlation());
            $this->outbox->record('DebitNoteReversed', 'DebitNote', $id, ['debitNoteId' => $id, 'reversalId' => $reversal->id, 'impactGraphHash' => $impactHash, 'resultingState' => $body['debit_note'], 'journalEntryIds' => $allJournalIds], $entityId, metadata: ['causation_id' => $causationId]);
            $this->commands->store($makerId, $entityId, 'debit_note_reverse:'.$id, $key, $hash, 201, $body);
    ...
    private function creditFx(?array $sourceReference, ?array $comparisonReference, string $amount): array|false
    {
        ...
        $calculated = $this->fx->calculateCredit($amount, (string) $sourceReference['rate'], (string) $comparisonReference['rate'], 'vendor', (int) $scale, $mode);
    ...
    private function reconflict(string $entityId, string $id, int $expected, ?DebitNote $note): DocumentActionResult
    {
        if ($note === null) {
            return $this->commands->error('not_found', 'The debit note was not found.', 404);
        }
